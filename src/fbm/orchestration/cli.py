...
from fbm.utils.csvout import write_csv  # writes tickets to gold CSV
...
def daily(season: int, week: int, league: str, config_path: str):
    ...
    # ---- Baseline model (configurable later; ratings default empty)
    model = BaselineModel(
        ratings={},          # all teams equal for now
        hfa_points=float(cfg.get("model", {}).get("hfa_points", 2.0)),
        sigma_diff=float(cfg.get("model", {}).get("sigma_diff", 13.0)),
        sigma_total=float(cfg.get("model", {}).get("sigma_total", 10.0)),
    )
    league_total_mean = float(cfg.get("model", {}).get("league_total_mean", 45.0))
    ...
    for r in rows:
        ...
        tickets.append({
            ...
            "edge": f"{edge_sp:+.4f}", "ev_per_dollar": f"{ev_sp:+.4f}",
        ...
        tickets.append({
            ...
            "edge": f"{edge_ou:+.4f}", "ev_per_dollar": f"{ev_ou:+.4f}",
